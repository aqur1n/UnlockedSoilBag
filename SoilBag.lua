...
function SoilBag.client_onCreate( self )
	print("---------------------------------------------")
	print("              MODDED BY AQUR1N               ")
	print("DISCORD: aqur1n#9546")
	print("DISCORD SERVER: https://discord.gg/v4hC2z4ZHh")
	print("-------------------SoilBag-------------------")
...
local function floor(int) -- aqur1n
	local f = math.floor(int)

	if int < (f + 0.25) then
		return f
	elseif int < (f + 0.75) then
		return (f + 0.5)
	else
		return (f + 1)
	end
...
function SoilBag.client_onEquippedUpdate( self, primaryState, secondaryState, forceBuildActive )
	if self.tool:isLocal() then
		...
		local valid, worldPos, worldNormal, result = SoilBag.constructionRayCast( { "terrainSurface" } )
		if valid then
			...
			if primaryState == sm.tool.interactState.start then
				...
				self:putSoil()
				if sm.physics.sphereContactCount( worldPos, 0.01, false, true ) > 0 and sm.physics.sphereContactCount( worldPos, 0.01, false, false ) == 0 then -- aqur1n
					local pos = result.pointWorld
					pos.z = pos.z + 1
					sm.projectile.projectileAttack( projectile_water, 0, pos, sm.vec3.new( 0.1, 0, 0 ), sm.localPlayer.getPlayer() )
				end
			end
